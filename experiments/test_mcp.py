...
def test_mcp_server(date="12-01", time_str="12:00", latitude=46, longitude=5.5):
    ...
    try:
        ...
        # Call reverse_geocode_azure_maps tool
        print("\nGetting location from Azure Maps...")
        ...
        reverse_geocode_response = send_request_and_get_response(reverse_geocode_request, "reverse_geocode")
        if not reverse_geocode_response:
            print("✗ No response from reverse_geocode_azure_maps tool.")
        else:
            print("✓ Reverse geocode response:")
            print(json.dumps(reverse_geocode_response, indent=2))
